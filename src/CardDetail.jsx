...
const CardDetail = ({ eventos }) => {
    ...
    return (
        ...
        className={` ${Context.isCardDetailOpen ? 'flex' : 'hidden'} fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 mt-10 bg-slate-50/90 w-[90vw] h-[70vh] rounded-3xl z-10 overflow-x-hidden`}>
            ...
            <ArrowSmallRightIcon className='absolute w-11 right-0 mt-[30vh] transition-transform delay-300' onClick={(event) => siguienteCard(event)}></ArrowSmallRightIcon>
            <ArrowSmallLeftIcon className='absolute w-11 l-0 mt-[30vh] transition-transform delay-300' onClick={(event) => anteriorCard(event)}></ArrowSmallLeftIcon>
            {Context.eventSelect && (
